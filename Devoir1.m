function [pcmNL INL alphaNL]=Devoir1(AngRot, vangulaire, forces, posNL)
    ...
    cm_nav_cyl = cm_cylindre(h_nav_cyl);
    cm_nav_cone = cm_cone(h_nav_cone) + [0 0 h_nav_cyl];
    cm_navette =...
        cm_compute([m_nav_cyl m_nav_cone], [cm_nav_cyl; cm_nav_cone]);
    
    i_nav_cyl = inertia_cylindre(m_nav_cyl, r_nav, h_nav_cyl);
    i_cone_nav = inertia_cone(m_nav_cone, r_nav, h_nav_cone);
    ...
    h_res_cyl = 46.9;
    h_res_top = 1/3 * h_res_cyl;
    h_res_bot = 2/3 * h_res_cyl;
    h_res_cone = 7.8;
    ...
    res_cone_offset = [0 r_nav + r_res h_res_cyl];
    
    cm_res_top = cm_cylindre(h_res_top) + res_top_offset;
    cm_res_bot = cm_cylindre(h_res_bot) + res_bot_offset;
    cm_res_cone = cm_cone(h_res_cone) + res_cone_offset;
    cm_reservoire = cm_compute(...
        [m_res_top m_res_bot m_res_cone],...
        [cm_res_top; cm_res_bot; cm_res_cone]...
    );
    
    i_res_top = inertia_cylindre(m_res_top, r_res, h_res_top);
    i_res_bot = inertia_cylindre(m_res_bot, r_res, h_res_bot);
    i_res_cone = inertia_cone(m_res_cone, r_res, h_res_cone);
    ...
    m_prop_cone = m_nav / (vol_prop_cyl + vol_prop_cone) * vol_prop_cone;
    
    prop_left_cyl_offset = [-r_res - r_prop, r_nav+r_res 0];
    prop_right_cyl_offset = [r_res + r_prop, r_nav+r_res 0];
    
    prop_left_cone_offset = [-r_res - r_prop, r_nav+r_res h_prop_cone];
    prop_right_cone_offset = [r_res + r_prop, r_nav+r_res h_prop_cone];
    
    cm_prop_cyl = cm_cylindre(h_prop_cyl);
    cm_prop_cone = cm_cone(h_prop_cone);
    
    cm_prop_left_cyl = cm_prop_cyl + prop_left_cyl_offset;
    cm_prop_right_cyl = cm_prop_cyl + prop_right_cyl_offset;
    cm_prop_left_cone = cm_prop_cone + prop_left_cone_offset;
    cm_prop_right_cone = cm_prop_cone + prop_right_cone_offset;
    ...
    i_prop_left_cyl = inertia_cylindre(m_prop_cyl, r_prop, h_prop_cyl);
    i_prop_right_cyl = i_prop_left_cyl;
    i_prop_left_cone = inertia_cone(m_prop_cone, r_prop, h_prop_cone);
    i_prop_right_cone = i_prop_left_cone;
