function [w1, isRefracted] =  A(r, w0, mCylindre, hCylindre, rCylindre, n_in, n_out)
  ...
  if (norm(w0_xy) ~= 0 || norm(normal_xy) ~= 0)
    phi_in = acos(dot(w0_xy,normal_xy) / (norm(w0_xy) * norm(normal_xy)));
    phi_out = asin(n_in * sin(phi_in) / n_out);
  end

  w0_xz = [w0(1) w0(3)]; 
  normal_xz = [normal(1) normal(3)];
  theta_in = acos(dot(w0_xz,normal_xz) / (norm(w0_xz) * norm(normal_xz)));
